...
**Status:** Implemented and covered by `src/pipeline/pipeline.test.ts`, `src/cli/args.test.ts`, runner wiring, and README provider/model docs.
...
- [x] **Step 1 (TDD): `withStageModel` in pipeline.ts.** Symmetric to `withStageProvider`:
  ```ts
...
- [x] **Step 2: args.ts.** Add options `provider-model: { type: 'string' }`, `review-model: { type: 'string' }`. Add `providerModel?: string; reviewModel?: string` to BOTH the run and watch Command members. Parse: `...(typeof values['provider-model'] === 'string' ? { providerModel: values['provider-model'] } : {})` and same for `review-model`, in both the run and watch return objects. USAGE: under the provider lines add:
...
- [x] **Step 3: runner deps.** `RunLinearIssueDeps` and `RunGithubIssueDeps` gain `providerModel?: string; reviewModel?: string`. In `runLinearIssue`/`runGithubIssue`, after building the base pipeline (and after the existing `withStageProvider` review-provider step), apply models:
  ```ts
...
- [x] **Step 4: cli/run.ts + cli/watch.ts.** Thread `cmd.providerModel`/`cmd.reviewModel` into the linear + github (issue/project) deps, exactly like `provider`/`reviewProvider` are threaded today (`...(cmd.providerModel !== undefined ? { providerModel: cmd.providerModel } : {})` for linearDeps; `if (cmd.providerModel !== undefined) deps.providerModel = cmd.providerModel;` for the github mutate-style deps; same for reviewModel; and in watch's linear deps object + buildGithubDeps).

- [x] **Step 5: README.** In the Providers/Models section note: `--provider-model` / `--review-model` pick the model per stage, independent of `--provider`/`--review-provider`; default is each provider's default model. Example: `vanguard run --linear TES-1 --provider-model opus --review-model haiku`.

- [x] **Step 6:** `pnpm typecheck && pnpm test` green. Commit: `git commit -am "feat: wybór modelu per etap (--provider-model / --review-model)"`.
